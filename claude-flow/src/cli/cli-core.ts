...
// Export enhanced CLI that supports plugins
export class ClaudeFlowWithPlugins extends CLI {
  private claudeFlowInstance: any;
  
  constructor(options: { plugins?: string[] } = {}) {
    const claudeFlowInstance = {
      agents: {},
      tasks: {},
      memory: {},
      mcp: {},
    ...
    super('claude-flow', 'Advanced AI Agent Orchestration System', claudeFlowInstance);
    this.claudeFlowInstance = claudeFlowInstance;
    
    // Auto-load plugins from constructor options
    if (options.plugins && this.pluginSystem) {
      this.pluginSystem.initialize().then(() => {
        options.plugins?.forEach(plugin => {
          this.pluginSystem!.loadPlugin(plugin).catch(err => {
  ...
  getClaudeFlow() {
    return this.claudeFlowInstance;
  }
}
